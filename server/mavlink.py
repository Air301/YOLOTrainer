...
class CustomSITL:
    ...
    def scan_open_mavlink_ips(self,ip_prefix='192.168.0.', port=5762, timeout=0.5):
        """扫描一个局域网段内哪些 IP 的指定端口是开放的"""
        open_ips = []
        for i in range(1, 255):
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                ...
                if result == 0:
                    print(f"[+] Found open MAVLink port on: {ip}:{port}")
                    open_ips.append(ip)
        return open_ips

    def connect_to_sitl(self, ARMING_CHECK, USE_UART):
        """连接到 SITL"""
        try:
            connection_successful = False
            # 判断是否是 Jetson
            if os.path.exists("/dev/ttyTHS1") and USE_UART:
                try:
                    print("尝试通过 Jetson 串口连接 /dev/ttyTHS1...")
                    self.connection = mavutil.mavlink_connection('/dev/ttyTHS1', baud=115200)
            ...
            # 如果串口失败，尝试通过 TCP 扫描 IP
            if not connection_successful:
                print("串口失败，尝试通过 TCP 连接...")
                # ip_candidates = self.scan_open_mavlink_ips('192.168.0.')
                # print("扫描完成")
                ip_candidates = False
                if not ip_candidates:
                    print("没有找到任何可连接的 SITL IP,设置ip为127.0.0.1")
                    target_ip='127.0.0.1'
                else:
                    target_ip = ip_candidates[0]
                print(f"连接目标 IP: {target_ip}")
                self.connection = mavutil.mavlink_connection(f'tcp:{target_ip}:5762')
                self.connection.wait_heartbeat(timeout=5)
                print("成功通过 TCP 连接 SITL")

            # 后续 MAVLink 参数设置等可以照旧继续
            ...
            while True:
                ...
                if msg is None:
                    break
            self.press_origin=self.connection.messages['SCALED_PRESSURE'].press_abs
            self.temp_origin=self.connection.messages['SCALED_PRESSURE'].temperature/100
            self.alt_origin=self.connection.messages['GLOBAL_POSITION_INT'].alt/1000
    ...
    def send_gps_input(self, lat, lon, alt):
        ...
        if self.last_lat ==0 or self.last_lon ==0 or self.last_alt ==0 or self.last_time ==0:
            self.last_lat = lat
            self.last_lon = lon
            self.last_alt = alt
            ...
            self.last_lon = lon
            self.last_alt = alt
            # self.logger.log(f"计算速度信息: vn={vn:.3f}, ve={ve:.3f}, vd={vd:.3f}，mavlink速度信息：vn={self.vn:.3f}, ve={self.ve:.3f}, vd={self.vd:.3f}")
    ...
    def get_global_position(self):
        if(abs(self.press_alt-self.current_alt)>4):
            self.logger.log(f"使用了气压高：{self.press_alt}，GPS高{self.current_alt}")
            return self.real_lat, self.real_lon, self.press_alt, self.global_position_lat, self.global_position_lon
        else:
    ...
    def refresh_msg(self):
        ...
        self.current_alt=self.connection.messages['GLOBAL_POSITION_INT'].alt/1000
        ...
        press_abs=self.connection.messages['SCALED_PRESSURE'].press_abs
        #根据压差计算高度
